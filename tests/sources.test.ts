...
import { SourcesImpl } from "../src/sources";
...
const testTransactionsIngCsvFormat = `20210629,Mr. G,NL00MAIN1234567890,NL00MAIN1234567892,GT,Debit,1000,Online Banking,Savings
...
describe("Sources", () => {
    test("should automatically detect the type of transactions data and load transactions", () => {
        for (var transactionsData in [
            ...
            expect(transactions[0].date).toEqual(new Date("2021-06-29"));
            expect(transactions[0].amount).toBe(-1000 * DECIMAL);
            expect(transactions[0].account).toBe("NL00MAIN1234567890");
            expect(transactions[0].contraAccount).toBe("NL00MAIN1234567892");
            expect(transactions[0].contraAccountName).toBe("Mr. G");
            expect(transactions[0].description).toBe("Savings");

            expect(transactions[1].date).toEqual(new Date("2021-06-28"));
            expect(transactions[1].amount).toBe(4000 * DECIMAL);
            expect(transactions[1].account).toBe("NL00MAIN1234567890");
            expect(transactions[1].contraAccount).toBe("NL01WORK0987654321");
            expect(transactions[1].contraAccountName).toBe("Company Inc.");
            expect(transactions[1].description).toBe("Salary for June 2020");
        }
    });
