<template>
    <div class="sources-tab full-size">
        <div class="sources-summary">
            <div>
                <span class="sources-summary-stat">{{ totalAccounts }}</span>
            ...
            <div>
                <span class="sources-summary-stat sources-summary-stat-wide">{{ totalTransactions }}</span>
        ...
        <div class="source-data-list">
            <div class="source-data-item" v-for="info of allSourceDataInfo">
...
export default {
    ...
    computed: {
        ...
        // Note: For now, these simply sum up the totals of each source data.
        //       In the future, it would be useful to remove duplicate
        //       accounts and transactions from these numbers.

        totalAccounts() {
            return (this as any).$root.$data.state.allSourceDataInfo()
                .map((info: SourceDataInfo) => info.nAccounts)
                .reduce((a: number, b: number) => a + b, 0);
        },

        totalTransactions() {
            return (this as any).$root.$data.state.allSourceDataInfo()
                .map((info: SourceDataInfo) => info.nTransactions)
                .reduce((a: number, b: number) => a + b, 0);
        }
    },
